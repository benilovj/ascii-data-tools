  ...
  module Record
    describe Record do
      ...
      it "should provide the type name" do
        Record.new(mock(Type, :name => "ABC"), nil).type_name.should == "ABC"
      end

      it "should provide access to its contents" do
        record = Record.new(mock(Type, :field_names => ["field1", "field2"]), ["xyz", "abc"])
        record["field1"].should == "xyz"
        record["field2"].should == "abc"        
      end
      
      it "should be representable as an array" do
        Record.new(mock(Type, :field_names => ["field1", "field2"]), ["xyz", "abc"]).to_a.should == [
          ["field1", "xyz"],
          ["field2", "abc"]
        ]
      end
      ...
      it "should have a string representation which includes the type name and key-value pairs" do
        record_type = mock(Type, :name => "ABC", :field_names => ["field1", "field2"])
      ...
      it "should be comparable to other records" do
        type = mock(Type, :field_names => ["field1", "field2"])
        another_type = mock(Type, :field_names => ["field1", "field3"])
      ...
      it "should raise an error when trying to access a non-existing field" do
        lambda {Record.new(mock(Type, :field_names => ["existing"]), ["xyz"])['non-existing'] }.should raise_error(/non-existing.*does not exist/)
      end
    end
  end
end